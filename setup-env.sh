...
pip list | grep -E "torch|transformer|lightning |accelerate|deepspeed|flash_attn|numpy|sentencepiece|eval|chris|prog|pydantic" > versions-pip.txt
...
mkdir -p output
mkdir -p output-lfs
ln -s /dlfs/jhryu/output-dl012 output-lfs-dl012  # for dl012
ln -s /dlfs/jhryu/output-dl026 output-lfs-dl026  # for dl026
